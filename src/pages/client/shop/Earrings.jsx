...
import { FlashIcon, SimpleMagic } from "../../../components/Icons";
    ...
    const fetchEarrings = async () => {
      try {
        const res = await axios.get(
          "http://localhost:6060/api/products/category/Earrings"
        );
  ...
  return (
    ...
    >
      <h1 className="text-2xl sm:text-3xl font-bold mb-8 text-center flex items-center justify-center gap-2">
        ✨ Earrings Collection
        <SimpleMagic className="w-6 h-6 sm:w-8 sm:h-8" />
      </h1>

      {/* Empty Products */}
      {products.length === 0 ? (
        <p className="text-center text-gray-500">No Earring products found.</p>
      ) : (
