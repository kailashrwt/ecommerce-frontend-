    ...
    const fetchBracelets = async () => {
      try {
        const res = await axios.get(
          "http://localhost:6060/api/products/category/Bracelets"
        );
  ...
  return (
    ...
    >
      <h1 className="text-2xl sm:text-3xl font-bold mb-8 text-center flex items-center justify-center gap-2">
        <ShoppingBag className="w-6 h-6 sm:w-8 sm:h-8" /> Bracelets Collection
      </h1>

      {/* No Products */}
      {products.length === 0 ? (
        <p className="text-center text-gray-500">No Bracelet products found.</p>
      ) : (
